...
class ShadcnLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final scaledTheme =
        themeMode == ThemeMode.dark || (themeMode == ThemeMode.system && platformBrightness == Brightness.dark) ? appScaling.scale(darkTheme ?? theme) : appScaling.scale(theme);
    return MultiModel(
      data: [
...
class ShadcnRectArcTween extends RectTween {
  ...
  void _initialize() {
    ...
    final _BorderRadiusCorner diagonal = _findMax<_BorderRadiusCorner>(_allDiagonals, (_BorderRadiusCorner d) => _diagonalSupport(centersVector, d));
